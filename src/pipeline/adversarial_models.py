...
class Baseline_adv(Adv_model, sk.base.BaseEstimator):
    ...
    def __init__(self, victim, constraints, metadata, constraint_correction, queue_size, total_step_num):
        ...
        if self.constraint_correction:
            guard_constraints = [constraint for constraint in constraints.relation_constraints if isinstance(constraint, BaseRelationConstraint)]
    ...
    @staticmethod
    def SEARCH_DIMS():
        return [[True,False], (1,100), (2,10000)] # NOTE: these are still chosen arbitrarily. May be subject to adjustment
    ...
    def attack(self, x):
        ...
        print(f"attack time {(time.time() - attack_timestamp)/60} minutes")
        print(f"for {x.shape[0]} samples")
        return adv_samples
    ...
    def adv_best_first_search(self, start, base_label, victim):
        ...
        while len(open_nodes) > 0 and current_iter < self.max_iterations:
            ...
            print(f"expand node time: {(time.time() - timestamp)/60} min")

            timestamp = time.time()
            perturbation_eval_results = self.node_eval_function(perturbations, start, base_label, victim)
            print(f"eval nodes time: {(time.time() - timestamp)/60} min")
    ...
    def generate_next_nodes(self, start_node):
        time_delta = 0
        ...
        print(f"expand nodes spent on constraint fixing: {time_delta/60} min")
